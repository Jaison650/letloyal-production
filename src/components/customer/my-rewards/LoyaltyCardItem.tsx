...
export default function LoyaltyCardItem({ card, phone }: { card: LoyaltyCard; phone: string }) {
  ...
  return (
    <div className={`relative pl-4 rounded-[16px] border bg-surface-1 shadow-ds overflow-hidden ${isUnlocked ? 'border-reward/40' : 'border-stroke'}`}>
      {/* Merchant colour strip — no per-merchant colour field on this card yet, so a fixed accent is used. */}
      <span aria-hidden className="absolute inset-y-0 left-0 w-1" style={{ background: 'var(--accent-default)' }} />
      <div className="flex items-center gap-3 px-4 pt-4 pb-2">
        <Link href={`/p/${card.merchant_slug}`} className="flex items-center gap-3 flex-1 min-w-0 hover:opacity-80 transition-opacity">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-ink truncate">{card.business_name}</p>
      ...
      <div className="px-4 pb-4">
        <p className="text-sm font-semibold text-ink flex items-center gap-1.5 mb-2">
          <Gift size={13} className="text-teal flex-shrink-0" />{card.reward_description}
        </p>
